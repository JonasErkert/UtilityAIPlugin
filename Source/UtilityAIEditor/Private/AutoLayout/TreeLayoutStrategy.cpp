...
{
	EdGraph = Cast<UEdGraph_UtilityAI>(_EdGraph);
	check(EdGraph != nullptr);

	EdGraph->RebuildUtilityAI();
	Graph = EdGraph->GetUtilityAI();
	check(Graph != nullptr);
	...
	{
		UUtilityAINode* RootNode = Graph->RootNodes[i];
...
void UTreeLayoutStrategy::InitPass(UUtilityAINode* RootNode, const FVector2D& Anchor)
{
	UEdNode_UtilityAINode* EdNode_RootNode = EdGraph->NodeMap[RootNode];
	...
	{
		UUtilityAINode* Child = RootNode->ChildrenNodes[i];
		UEdNode_UtilityAINode* EdNode_ChildNode = EdGraph->NodeMap[Child];
		if (i > 0)
		{
			UUtilityAINode* PreChild = RootNode->ChildrenNodes[i - 1];
			UEdNode_UtilityAINode* EdNode_PreChildNode = EdGraph->NodeMap[PreChild];
...
bool UTreeLayoutStrategy::ResolveConflictPass(UUtilityAINode* Node)
{
	bool HasConflict = false;
	for (int32 i = 0; i < Node->ChildrenNodes.Num(); ++i)
	{
		UUtilityAINode* Child = Node->ChildrenNodes[i];
	...
	{
		UUtilityAINode* ParentNode = Node->ParentNodes[i];
		for (int32 j = 0; j < ParentNode->ChildrenNodes.Num(); ++j)
		{
			UUtilityAINode* LeftSibling = ParentNode->ChildrenNodes[j];
			if (LeftSibling == Node)
				break;
...
bool UTreeLayoutStrategy::ResolveConflict(UUtilityAINode* LRoot, UUtilityAINode* RRoot)
{
	TArray<UEdNode_UtilityAINode*> RightContour, LeftContour;
	...
	if (MaxOverlapDistance > 0)
	{
		...
		TArray<UUtilityAINode*> ParentNodes = RRoot->ParentNodes;
		TArray<UUtilityAINode*> NextParentNodes;
...
void UTreeLayoutStrategy::GetLeftContour(UUtilityAINode* RootNode, int32 Level, TArray<UEdNode_UtilityAINode*>& Contour)
{
	UEdNode_UtilityAINode* EdNode_Node = EdGraph->NodeMap[RootNode];
...
void UTreeLayoutStrategy::GetRightContour(UUtilityAINode* RootNode, int32 Level, TArray<UEdNode_UtilityAINode*>& Contour)
{
	UEdNode_UtilityAINode* EdNode_Node = EdGraph->NodeMap[RootNode];
	if (Level >= Contour.Num())
	{
...
void UTreeLayoutStrategy::ShiftSubTree(UUtilityAINode* RootNode, const FVector2D& Offset)
{
	UEdNode_UtilityAINode* EdNode_Node = EdGraph->NodeMap[RootNode];
...
void UTreeLayoutStrategy::UpdateParentNodePosition(UUtilityAINode* ParentNode)
{
	UEdNode_UtilityAINode* EdNode_ParentNode = EdGraph->NodeMap[ParentNode];
	if (ParentNode->ChildrenNodes.Num() % 2 == 0)
	{
		UEdNode_UtilityAINode* FirstChild = EdGraph->NodeMap[ParentNode->ChildrenNodes[0]];
		UEdNode_UtilityAINode* LastChild = EdGraph->NodeMap[ParentNode->ChildrenNodes.Last()];
	...
	{
		UEdNode_UtilityAINode* MidChild = EdGraph->NodeMap[ParentNode->ChildrenNodes[ParentNode->ChildrenNodes.Num() / 2]];
